<script lang="ts">
  ...
  let onValueChange = (val: string) => {
    ...
    // TODO: Add logic for language across app
...
<div class="flex gap-x-2 lg:gap-x-2.5">
  <i class={`${selectedLang.iconPack} ${selectedLang.flag}`}></i>
  <Select.Root type="single" name="language" bind:value {onValueChange}>
    ...
    <Select.Content>
      {#each languages as lang (lang.id)}
        <Select.Item value={lang.language}>
          {lang.language}
        </Select.Item>
      {/each}
    </Select.Content>
